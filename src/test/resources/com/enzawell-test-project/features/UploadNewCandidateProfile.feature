...
Scenario Outline: Upload New Candidate Profile
  ...
  Scenario Outline: Upload New Candidate Profile
    ...
    Examples:
      | candidate_profile        |
      | valid_candidate_profile_1 |
      | valid_candidate_profile_2 |
...
Scenario Outline: Upload New Candidate Profile
  When the recruiter clicks on the upload icon
  And the recruiter browses and selects a valid candidate profile file with UTF-8 encoding
  And the recruiter clicks on the Upload button
  Then the upload process completes successfully
  And a success confirmation message is displayed to the user
...
Examples:
  ...
  Scenario Outline: Upload New Candidate Profile
    When I click on the upload (cloud) icon
    And I browse and select an unsupported file type <unsupported_file_type>
    And I click on the Upload button
    Then an error message is displayed indicating the file type is not supported
    And the upload process does not initiate
...
Scenario Outline: Upload New Candidate Profile
  ...
  And the recruiter does not select any file
  And the recruiter clicks on the Upload button
  Then the system displays an error message indicating that no file was selected
  And the upload process does not initiate
...
Examples:
  ...
  Scenario Outline: Upload New Candidate Profile
    ...
    And I browse and select a valid candidate profile file named "<filename>"
    And I click on the Upload button
